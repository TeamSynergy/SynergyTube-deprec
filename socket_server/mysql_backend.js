...
exports.user.exists = function(login_name, email, fn){
	sql.query("SELECT COUNT(*) AS '_c' FROM tblUser WHERE login_name = " + sql.escape(login_name) + " OR email = " + sql.escape(email), function(err, rows){
		if(err)
			...
			return fn(null, rows[0]._c > 0);
	});
};

exports.user.create = function(login_name, email, password_hash, validate_hash){
	sql.query("INSERT INTO tblUser (login_name, display_name, email, strategy, hash, is_valid, validate_hash) VALUES (" + sql.escape(login_name.toLowerCase()) + ", " + sql.escape(login_name) + ", " + sql.escape(email) + ", 'local', " + sql.escape(password_hash) + ", 0, " + sql.escape(validate_hash) + ")", function(err){
		return fn(err, null);
	});
};

exports.user.session.destroy = function(session_id, login_name, fn){
...
exports.user.session.create = function(session_id, login_name, fn){
...
exports.channel.chat.getMore = function(channel_id, count, last_stamp, fn){
	sql.query("SELECT timestamp, content, display_name FROM tblMessages INNER JOIN tblUser ON tblUser._id = tblMessages.user_id WHERE channel_id = " + sql.escape(channel_id) + " AND timestamp < " + sql.escape(new Date(last_stamp)) + " ORDER BY timestamp DESC LIMIT 0, " + sql.escape(count), function(err, rows){
		if(err)
			return fn(err, null);
		else
			return fn(null, rows);
