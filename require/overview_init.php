<?php
    ...
    $count_favs = mysqli_fetch_object($con->query("SELECT COUNT(*) AS '_c' FROM relFavourites WHERE channel_id = '".$row->_id."'"))->_c ?:0;

		$count_media = mysqli_fetch_object($con->query("SELECT COUNT(*) AS '_c' FROM tblMedia WHERE channel_id = '".$row->_id."'"))->_c ?:0;

		$count_unique_views = mysqli_fetch_object($con->query("SELECT COUNT(DISTINCT ip_hash) AS '_c' FROM tblTracking WHERE channel_id = '".$row->_id."'"))->_c ?:0;
		$url = "";
      ...
      <div class="preview">
        <a href="<?php pe($url)?>">
          <div class="caption-wrapper">
            <img src="//img.youtube.com/vi/<?php p($now_playing->url)?:'duh' ?>/mqdefault.jpg"/>
